...
def createMappingTable(pRowFilterIG, pRowFilter, pHeading, pOutputFile, pIntroSpreadsheet, pSpreadsheet)
    ...
    profiles.each do |(x, y)| 
        ...
        CSV.foreach(pSpreadsheet) do |row|
            ...
            print row[IJE_PROFILE_COL].to_s + " : " + row[IJE_NAME_COL].to_s + "\n"
            # Two of the DecedentUsualWork fields are inputs, and all are coding outputs
            next if y.to_s != "Coding-Work" && row[IJE_PROFILE_COL] == "DecedentUsualWork" && (row[IJE_NAME_COL] != "OCCUP" && row[IJE_NAME_COL] != "INDUST")
            print row[IJE_PROFILE_COL].to_s + " : " + row[IJE_NAME_COL].to_s + "\n"
